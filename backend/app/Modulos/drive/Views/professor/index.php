...
(function() {
    ...
    document.getElementById('btn-new-folder')?.addEventListener('click', function() { document.getElementById('modal-new-folder').classList.remove('hidden'); });
    document.getElementById('btn-upload')?.addEventListener('click', function() { document.getElementById('input-file').click(); });
            ...
            fetch(formUpload.action, { method: 'POST', body: formData, redirect: 'manual' })
                .then(function(res) {
                    if (res.type === 'opaqueredirect' || res.status === 0 || res.status === 302) {
                        ...
                        window.location.href = url || (currentFolderId ? baseUrl + '/folder/' + currentFolderId : baseUrl);
    ...
    document.getElementById('input-file')?.addEventListener('change', function() {
        if (this.files.length && formUpload) formUpload.dispatchEvent(new Event('submit', { cancelable: true }));
    });
    document.querySelectorAll('.modal-close').forEach(function(btn) { btn.addEventListener('click', function() { document.querySelectorAll('[id^="modal-"]').forEach(function(m) { m.classList.add('hidden'); }); }); });
    ...
    document.getElementById('share-search')?.addEventListener('input', function() {
        ...
        shareSearchTimer = setTimeout(function() {
            fetch(baseUrl + '/search-users?q=' + encodeURIComponent(q)).then(function(r) { return r.json(); }).then(function(data) {
                resultsEl.innerHTML = '';
                (data.users || []).forEach(function(u) {
                    ...
                    li.addEventListener('click', function() {
                        ...
                        form.innerHTML = '<input type="hidden" name="_token" value="' + csrf + '"><input type="hidden" name="item_id" value="' + itemId + '"><input type="hidden" name="shared_with_id" value="' + u.id + '"><input type="hidden" name="shared_with_type" value="' + u.tipo + '"><input type="hidden" name="permission" value="view">';
    ...
    document.querySelectorAll('.drive-action-share').forEach(function(btn) {
        btn.addEventListener('click', function(e) {
            ...
            fetch(baseUrl + '/share-list/' + row.dataset.itemId).then(function(r) { return r.json(); }).then(function(data) {
