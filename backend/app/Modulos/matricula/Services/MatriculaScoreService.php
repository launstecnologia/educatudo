...
class MatriculaScoreService
{
    ...
    public function buscarScore(int $alunoId, int $ciclo): ?array
    {
        return $this->db->fetch(
            "SELECT * FROM matricula_processos_scores WHERE aluno_id = ? AND ciclo = ?",
            [$alunoId, $ciclo]
        ) ?: null;
    }
    ...
    public function listarPorFaixa(int $ciclo, string $faixa = ''): array
    {
        ...
        return $this->db->fetchAll(
            "SELECT s.*, a.nome AS aluno_nome, t.nome AS turma_nome, t.serie AS turma_serie
             FROM matricula_processos_scores s
    ...
    private function persistir(int $alunoId, int $ciclo, int $score, string $faixa, array $sinais, array $motivos): void
    {
        ...
        $existing = $this->db->fetch(
            "SELECT id FROM matricula_processos_scores WHERE aluno_id = ? AND ciclo = ?",
            [$alunoId, $ciclo]
        );
        if ($existing) {
            $this->db->update(
                "UPDATE matricula_processos_scores SET score=?, faixa=?, freq_n=?, desemp_n=?, inad_n=?, engaj_n=?, tempo_n=?, motivos=?, calculado_em=NOW()
            ...
            $this->db->insert(
                "INSERT INTO matricula_processos_scores (aluno_id, ciclo, score, faixa, freq_n, desemp_n, inad_n, engaj_n, tempo_n, motivos)
                 VALUES (?,?,?,?,?,?,?,?,?,?)",
                [
