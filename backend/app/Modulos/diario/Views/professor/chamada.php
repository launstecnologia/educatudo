...
$voltarUrl = $origem === 'diario'
    ? URL . '/professor/diarios/abrir?turma_id=' . (int) $aula['turma_id'] . '&materia_id=' . (int) $aula['materia_id'] . '&aba=aulas'
    : URL . '/professor/diario?data=' . urlencode($aula['data_aula']);
$voltarLabel = $origem === 'diario' ? '← Voltar ao diário' : '← Voltar à agenda do dia';
$moduloOcorrencias = !class_exists('LayoutHelper') || LayoutHelper::isModuleEnabled('ocorrencias');
?>
...
<style>
    ...
    .diario-select:focus { outline: none; border-color: #7c3aed; box-shadow: 0 0 0 3px rgba(124,58,237,.16); }
...
<div class="max-w-7xl mx-auto space-y-6">
    <?php
    $flashDiario = $_SESSION['flash_message'] ?? null;
    $flashDiarioTipo = $_SESSION['flash_type'] ?? 'info';
    if ($flashDiario) {
        unset($_SESSION['flash_message'], $_SESSION['flash_type']);
    }
    ?>
    <?php if ($flashDiario): ?>
        <div class="rounded-lg px-4 py-3 text-sm <?= $flashDiarioTipo === 'error' ? 'bg-red-50 text-red-800 border border-red-200' : 'bg-green-50 text-green-800 border border-green-200' ?>">
            <?= htmlspecialchars((string) $flashDiario) ?>
        </div>
    <?php endif; ?>
    <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
        <div>
            <a href="<?= htmlspecialchars($voltarUrl) ?>" class="text-purple-700 hover:underline text-sm"><?= htmlspecialchars($voltarLabel) ?></a>
            <h1 class="text-3xl font-bold text-gray-900 mt-2"><?= htmlspecialchars($aula['materia_nome']) ?> — <?= htmlspecialchars($aula['turma_nome']) ?></h1>
            <p class="text-gray-600 mt-1"><?= date('d/m/Y', strtotime($aula['data_aula'])) ?> · <?= substr($aula['horario_de'], 0, 5) ?>–<?= substr($aula['horario_ate'], 0, 5) ?></p>
        </div>
    ...
    <form method="post" action="<?= URL ?>/professor/diario/salvar" id="formChamada" class="space-y-6">
        ...
        <section class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4">
            ...
            <?php if ($planoAtual > 0 && !empty($aula['plano_titulo'])): ?>
                ...
                <a href="<?= URL ?>/professor/planos-aula/visualizar/<?= $planoAtual ?>" class="inline-block text-sm text-purple-700 hover:underline" target="_blank" rel="noopener">Ver plano completo ↗</a>
            <?php elseif ($planos === []): ?>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <?php if ($moduloOcorrencias): ?>
                                <th class="px-4 py-3 text-left text-xs uppercase text-gray-500">Ocorrência</th>
                                <?php endif; ?>
                            </tr>
                        </thead>
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <button type="button"
                                        ...
                                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors <?= $obs !== '' ? 'border-purple-300 text-purple-700 bg-purple-50 hover:bg-purple-100' : 'border-gray-300 text-gray-500 bg-white hover:bg-gray-50' ?>">
                                ...
                                <?php if ($moduloOcorrencias): ?>
                                <td class="px-4 py-3">
                                    <a href="<?= URL ?>/professor/ocorrencias/nova?aula_id=<?= (int) $aula['id'] ?>&aluno_id=<?= $aid ?><?= $origem === 'diario' ? '&origem=diario' : '' ?>"
                                       class="text-xs font-semibold text-purple-700 hover:underline">Registrar</a>
                                </td>
                                <?php endif; ?>
                            </tr>
                        <?php endforeach; ?>
        ...
        <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 pb-8">
            ...
            <button type="submit" name="acao" value="finalizar" onclick="return confirm('Finalizar esta chamada? Os dados continuarão registrados no histórico.')" class="px-5 py-2.5 bg-purple-600 text-white rounded-lg font-semibold hover:bg-purple-700">Finalizar chamada</button>
        </div>
    </form>
</div>
...
<div id="modalObs" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 hidden">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 flex flex-col max-h-[90vh]">
        ...
        <div class="flex gap-3 px-6 py-4 border-t border-gray-100">
            ...
            <button type="button" onclick="salvarObs()" class="flex-1 px-4 py-2.5 bg-purple-600 text-white rounded-lg text-sm font-semibold hover:bg-purple-700"><i class="fa-solid fa-check mr-2"></i>Confirmar</button>
        </div>
    </div>
</div>
...
function salvarObs() {
    ...
    if (btn) {
        if (html !== '') {
            btn.className = btn.className.replace('border-gray-300 text-gray-500 bg-white hover:bg-gray-50', 'border-purple-300 text-purple-700 bg-purple-50 hover:bg-purple-100');
            btn.innerHTML = '<i class="fa-solid fa-pen-to-square text-xs"></i> Ver obs.';
        } else {
            btn.className = btn.className.replace('border-purple-300 text-purple-700 bg-purple-50 hover:bg-purple-100', 'border-gray-300 text-gray-500 bg-white hover:bg-gray-50');
...
document.addEventListener('keydown', function(e) { if (e.key === 'Escape') fecharObs(); });
document.getElementById('modalObs').addEventListener('click', function(e) { if (e.target === this) fecharObs(); });
</script>
