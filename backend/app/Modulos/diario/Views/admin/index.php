...
$abaAtual = (($aba ?? 'pendentes') === 'fechados') ? 'fechados' : 'pendentes';
$filtrosAtivos = 0;
...
if ($status_filtro !== '') {
    $tabQuery['status'] = $status_filtro;
}
$exportQuery = http_build_query($tabQuery);
$pendentesUrl = URL . '/admin/diario?' . http_build_query(array_merge($tabQuery, ['aba' => 'pendentes']));
$fechadosUrl = URL . '/admin/diario?' . http_build_query(array_merge($tabQuery, ['aba' => 'fechados']));
$limparUrl = URL . '/admin/diario?aba=' . $abaAtual;
$totalFechados = (int) ($total_fechados ?? 0);
ob_start();
?>
...
$page_header_subtitle = $abaAtual === 'fechados'
    ? 'Bimestres já fechados pelos professores. Visualize o diário ou reabra se precisar corrigir.'
    : 'Acompanhe chamadas pendentes, finalizadas e aulas não realizadas.';
...
<div class="flex flex-wrap gap-2 text-sm mb-4">
    <a href="<?= htmlspecialchars($pendentesUrl) ?>"
       class="inline-flex items-center px-3 py-1.5 rounded-full <?= $abaAtual === 'pendentes' ? 'bg-primary text-primary' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' ?>">
        Pendentes
    </a>
    <a href="<?= htmlspecialchars($fechadosUrl) ?>"
       class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full <?= $abaAtual === 'fechados' ? 'bg-primary text-primary' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' ?>">
        Fechados
        <?php if ($totalFechados > 0): ?>
            <span class="inline-flex items-center justify-center min-w-[1.25rem] h-5 px-1.5 rounded-full text-xs font-semibold <?= $abaAtual === 'fechados' ? 'bg-white/20' : 'bg-gray-200 text-gray-700' ?>"><?= $totalFechados ?></span>
        <?php endif; ?>
    </a>
</div>

<?php if ($abaAtual === 'pendentes'): ?>
                ...
                <?php if (empty($aulas)): ?>
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                            ...
                            <?php if ($filtrosAtivos > 0): ?>
                                <a href="<?= htmlspecialchars($limparUrl) ?>" class="inline-block mt-3 text-sm text-blue-600 hover:text-blue-800">Limpar filtros</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php else: ?>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <?php foreach (['Turma', 'Componente', 'Professor', 'Ano/Bimestre', 'Fechado em', 'Ações'] as $header): ?>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><?= htmlspecialchars($header) ?></th>
                    <?php endforeach; ?>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
                <?php if (empty($fechamentos_fechados)): ?>
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                            <i class="fa-solid fa-lock text-4xl text-gray-300 mb-4"></i>
                            <p>Nenhum diário fechado<?= $filtrosAtivos > 0 ? ' para estes filtros' : '' ?>.</p>
                            <?php if ($filtrosAtivos > 0): ?>
                                <a href="<?= htmlspecialchars($limparUrl) ?>" class="inline-block mt-3 text-sm text-blue-600 hover:text-blue-800">Limpar filtros</a>
                            <?php endif; ?>
                        </td>
                ...
                <?php else: ?>
                    <?php foreach ($fechamentos_fechados as $f): ?>
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 text-sm text-gray-700"><?= htmlspecialchars((string) $f['turma_nome']) ?></td>
                            <td class="px-6 py-4 text-sm text-gray-700"><?= htmlspecialchars((string) $f['materia_nome']) ?></td>
                            <td class="px-6 py-4 text-sm text-gray-700"><?= htmlspecialchars((string) $f['professor_nome']) ?></td>
                            <td class="px-6 py-4 text-sm text-gray-700"><?= (int) $f['ano_letivo'] ?> · <?= (int) $f['bimestre'] ?>º bimestre</td>
                            <td class="px-6 py-4 text-sm text-gray-500"><?= !empty($f['fechado_em']) ? date('d/m/Y H:i', strtotime((string) $f['fechado_em'])) : '—' ?></td>
                            <td class="px-6 py-4">
                                <?php ob_start(); ?>
                                <a href="<?= URL ?>/admin/diario/fechado?id=<?= (int) $f['id'] ?>"
                                ...
                                <form method="post" action="<?= URL ?>/admin/diario/reabrir" onsubmit="return confirm('Reabrir este diário? O professor poderá editar chamadas novamente.')">
                                    ...
                                    <button type="submit" class="flex items-center gap-2 w-full text-left px-4 py-2 text-sm text-amber-700 hover:bg-amber-50">
                                        <i class="fa-solid fa-lock-open text-amber-500 w-4 text-center"></i> Reabrir
                                    </button>
                                </form>
                                <?php
                                $row_actions_dropdown_items = ob_get_clean();
                                $row_actions_dropdown_id = 'row-fechamento-' . (int) $f['id'];
                                include dirname(__DIR__, 4) . '/Views/admin/_partials/row_actions_dropdown.php';
                                ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<aside id="filterDrawer" class="fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl z-50 transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col" aria-hidden="true">
    ...
    <form method="get" action="<?= URL ?>/admin/diario" class="flex flex-col flex-1 overflow-hidden">
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex gap-3 bg-gray-50">
            <a href="<?= htmlspecialchars($limparUrl) ?>" class="flex-1 inline-flex items-center justify-center px-4 py-2.5 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors">Limpar</a>
            <button type="submit" class="flex-1 px-4 py-2.5 bg-primary text-primary rounded-lg text-sm font-semibold hover:opacity-90 transition-colors">Aplicar filtros</button>
        </div>
    </form>
</aside>
