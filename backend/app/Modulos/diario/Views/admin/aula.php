...
$fechamentoId = (int) ($fechamento_id ?? 0);
$voltarUrl = $fechamentoId > 0
    ? URL . '/admin/diario/fechado?id=' . $fechamentoId . '&aba=aulas'
    : URL . '/admin/diario?' . http_build_query(['inicio' => date('Y-m-01', strtotime((string) $aula['data_aula'])), 'fim' => $aula['data_aula']]);
?>
<div class="mb-6">
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden divide-y divide-gray-200">
    <section class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div>
                <span class="block text-xs font-medium text-gray-500 uppercase tracking-wider">Tipo da aula</span>
                <span class="mt-1 block text-sm text-gray-900"><?= htmlspecialchars(ClassDiaryService::tipoAulaLabel((string) ($aula['tipo_aula'] ?? 'regular'))) ?></span>
            </div>
    ...
    <section class="p-6 space-y-4">
        ...
        <?php endif; ?>
        <?php if (!empty($aula['evento_titulo'])): ?>
        <div class="rounded-lg border border-gray-200 bg-gray-50/70 p-4">
            <span class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Evento de prova/nota relacionado</span>
            <p class="text-sm font-semibold text-gray-900"><?= htmlspecialchars((string) $aula['evento_titulo']) ?></p>
            <p class="mt-1 text-sm text-gray-600">
                <?= !empty($aula['evento_data']) ? date('d/m/Y', strtotime((string) $aula['evento_data'])) : '' ?>
                — a nota permanece no evento, não neste diário.
            </p>
        </div>
        <?php endif; ?>
    </section>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ocorrência</th>
                        <?php endif; ?>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php foreach (($aula['frequencias'] ?? []) as $f):
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-3 text-sm">
                                <a href="<?= URL ?>/admin/ocorrencias/nova?aula_id=<?= (int) $aula['id'] ?>&aluno_id=<?= (int) ($f['aluno_id'] ?? 0) ?>"
                    ...
                    <?php if (empty($aula['frequencias'])): ?>
                        <tr><td colspan="<?= $moduloOcorrencias ? 4 : 3 ?>" class="px-6 py-8 text-center text-sm text-gray-500">Nenhum aluno ativo nesta turma.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
