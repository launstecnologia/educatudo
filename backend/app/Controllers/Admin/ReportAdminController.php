...
class ReportAdminController extends AdminBaseController
{
    ...
    public function censo()
    {
        ...
        $colsAlunos = $this->colunasExistentes('alunos');
        $temUnidades = $this->db->fetch("SHOW TABLES LIKE 'unidades'") !== false;
        $temUnidadeCol = isset($colsAlunos['unidade_id']);
        $temCodigoInep = isset($colsAlunos['codigo_inep']);
        ...
        if ($temUnidades) {
            try {
                $unidades = $this->db->fetchAll("SELECT id, nome FROM unidades WHERE ativo = 1 ORDER BY nome ASC") ?: [];
            } catch (\Throwable $e) {
                $unidades = [];
            }
        }
        ...
        $selCodigoInep = $temCodigoInep ? 'a.codigo_inep' : 'NULL';
        $selNomeMae = isset($colsAlunos['nome_mae']) ? 'a.nome_mae' : 'NULL';
        $selCpf = isset($colsAlunos['cpf']) ? 'a.cpf' : 'NULL';
        $selDataNasc = isset($colsAlunos['data_nasc']) ? 'a.data_nasc' : 'NULL';

        $alunos = $this->db->fetchAll(
            "SELECT a.id, a.nome,
                    {$selCodigoInep} AS codigo_inep,
                    {$selNomeMae} AS nome_mae,
                    {$selCpf} AS cpf,
                    {$selDataNasc} AS data_nasc,
        ...
        $semCpf = 0;
        foreach ($alunos as $a) {
            if (trim((string) ($a['codigo_inep'] ?? '')) !== '') {
                $comInep++;
            }
            if (trim((string) ($a['nome_mae'] ?? '')) === '') {
                $semMae++;
            }
            if (trim((string) ($a['cpf'] ?? '')) === '') {
                $semCpf++;
            }
        }

        $this->viewWithLayout('admin', 'admin/reports/censo', [
            'title' => 'Censo / INEP - EducaTudo',
            'user' => $user,
            'current_page' => 'reports_censo',
            'alunos' => $alunos,
            'unidades' => $unidades,
            'unidade_id' => $unidadeId,
            'tem_unidades' => $temUnidades && $temUnidadeCol,
            'tem_codigo_inep' => $temCodigoInep,
            'schema_pronto' => $temCodigoInep,
            'stats' => [
                'total' => $total,
                'com_inep' => $comInep,
                'sem_inep' => $total - $comInep,
                'sem_mae' => $semMae,
                'sem_cpf' => $semCpf,
            ],
