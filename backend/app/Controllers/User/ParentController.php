...
class ParentController extends BaseController
{
    ...
    {
        if ($alunoId <= 0 || $turmaId <= 0 || !$this->db->tableExists('jornadas')) {
            return [];
        }

        $hasMateriaId = $this->hasColumn('jornadas', 'materia_id');
        ...
        $joinMateria = $hasMateriaId ? "LEFT JOIN materias m ON m.id = j.materia_id" : "";
        $materiaExpr = $hasMateriaId ? "m.nome" : "NULL";
        ...
        if ($anoLetivo !== null && $anoLetivo > 0) {
            $whereExtras .= " AND YEAR(j.created_at) = :ano_letivo";
            $params['ano_letivo'] = $anoLetivo;
        }
        if ($bimestre !== null && $bimestre >= 1 && $bimestre <= 4) {
            $mesIni = (($bimestre - 1) * 3) + 1;
            $mesFim = $mesIni + 2;
            $whereExtras .= " AND MONTH(j.created_at) BETWEEN :mes_ini AND :mes_fim";
            $params['mes_ini'] = $mesIni;
        ...
        foreach ($rows as &$row) {
            ...
            // Expirado: passou do prazo (data_fim/hora_fim na estrutura) e aluno não iniciou.
            $row['expirada'] = false;
            $fimTs = null;
    ...
    public function filhoDetalhes($id)
    {
        ...
        $jornadas = $this->db->fetchAll(
            "SELECT j.*, p.nome as professor_nome
             FROM jornadas j
    ...
    public function jornadasFilho($id)
    {
        ...
        $jornadas = $this->getJornadasDetalhadasAluno((int) $filho['id'], (int) ($filho['turma_id'] ?? 0), $anoLetivo, $bimestre);
        $kpisJornadas = $this->buildKpisJornadas($jornadas);

        $anosBase = $this->db->fetchAll(
            "SELECT DISTINCT YEAR(j.created_at) AS ano
             FROM jornadas j
             WHERE j.turma_id = :turma_id
             ORDER BY ano DESC",
            ['turma_id' => (int) ($filho['turma_id'] ?? 0)]
        );
        $anosDisponiveis = [];
        foreach ((array) $anosBase as $rAno) {
            $a = (int) ($rAno['ano'] ?? 0);
            if ($a > 0) {
                $anosDisponiveis[] = $a;
            }
