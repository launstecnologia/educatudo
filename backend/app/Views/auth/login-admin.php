...
<head>
    ...
    <title>Login Administração - EducaTudo</title>
    ...
    ?>
    <?php
        $pwaThemeColor = LayoutHelper::get('pwa_admin_theme_color', LayoutHelper::get('primary_color', '#a855f7'));
        $pwaIcon192 = LayoutHelper::get('pwa_admin_icon_192', LayoutHelper::getLogo1x1Url());
    ?>
...
<body class="bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50 min-h-screen flex flex-col items-center justify-center p-4 md:p-6">
    <div class="max-w-md w-full">
        <!-- Login Form - no mobile compacto para caber inteiro na tela -->
        <div class="bg-white py-6 px-4 md:py-8 md:px-6 shadow-xl rounded-2xl border">
            <!-- Logo e Título dentro do card -->
            <div class="text-center mb-6 md:mb-8">
                <?php if (LayoutHelper::getContextualLogo('login', 'h-12 md:h-16 w-auto mx-auto', 'Logo EducaTudo')): ?>
                    <div class="mb-3 md:mb-4">
                        <?= LayoutHelper::getContextualLogo('login', 'h-12 md:h-16 w-auto mx-auto', 'Logo EducaTudo') ?>
                    </div>
                <?php else: ?>
                    <div class="w-12 h-12 md:w-16 md:h-16 bg-primary rounded-2xl flex items-center justify-center mx-auto mb-3 md:mb-4 shadow-lg">
                        <span class="text-white font-bold text-xl md:text-2xl">⚙️</span>
                    </div>
                <?php endif; ?>
                <h2 class="text-base md:text-lg text-gray-600 mb-2">Painel da Escola</h2>
            </div>
            ...
            <form id="loginForm" action="<?= URL ?>/login" method="post" class="space-y-4 md:space-y-6">
                ...
                <input type="hidden" name="tipo" value="admin_escola">
                ...
                <div>
                    ...
                    <input id="login" name="login" type="email" required
                           class="w-full px-4 py-2 md:py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-colors text-base"
                ...
                <div>
                    ...
                    <div class="relative">
                        <input id="senha" name="senha" type="password" required
                               class="w-full px-4 py-2 md:py-3 pr-12 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-colors text-base"
                ...
                <div>
                    <button type="submit" 
                            class="w-full flex justify-center py-2 md:py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-semibold btn-primary-custom focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-all duration-200">
    ...
    <!-- Footer - compacto no mobile para caber na tela -->
    <footer class="mt-4 md:mt-6 py-3 md:py-4 text-center border-t border-gray-200 flex-shrink-0">
        <p class="text-xs md:text-sm text-gray-900 mb-1">Todos os direitos reservados Educatudo</p>
        ...
        <p class="text-xs md:text-sm text-gray-900 mt-2">
