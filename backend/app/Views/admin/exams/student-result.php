...
<div class="mb-8">
    <div class="flex justify-between items-center">
        ...
        <div>
            <a href="<?= URL ?>/admin/provas" class="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700">
                Voltar
            </a>
        </div>
    </div>
...
<div class="bg-white rounded-xl shadow-lg p-6 mb-6">
    <div class="text-center">
        <h3 class="text-lg font-semibold text-gray-700 mb-4">Nota do Aluno</h3>
        <div class="text-5xl font-bold text-blue-600 mb-2">
            <?= number_format($realizacao['nota'], 2, ',', '.') ?>
        </div>
        <p class="text-gray-600">
            de <?= number_format($prova['valor_total'], 2, ',', '.') ?> pontos
        </p>
        <div class="mt-4">
            <?php 
            $percentual = ($realizacao['nota'] / $prova['valor_total']) * 100;
            $cor = $percentual >= 70 ? 'green' : ($percentual >= 50 ? 'yellow' : 'red');
            ?>
            <span class="px-4 py-2 rounded-full text-white font-semibold bg-<?= $cor ?>-600">
...
<div class="bg-white rounded-xl shadow-lg p-6 mb-6">
    ...
    <?php if (!empty($realizacao['marcacao_final_key'])): ?>
    <div class="mt-4 pt-4 border-t border-gray-200">
        <a href="<?= htmlspecialchars(URL . '/media/serve?type=provas_marcacao_final&key=' . rawurlencode($realizacao['marcacao_final_key'])) ?>" target="_blank" rel="noopener" class="inline-flex items-center text-blue-600 hover:text-blue-800 font-medium">
            Ver comprovante de marcação
            <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
        </a>
    </div>
    <?php endif; ?>
    <?php if (!empty($realizacao['finalizado_em']) && isset($prova['id']) && isset($aluno['id'])): ?>
    <?php $baseProvas = (isset($user['tipo']) && $user['tipo'] === 'professor') ? '/professor/provas' : '/admin/provas'; ?>
    <div class="mt-4 pt-4 border-t border-gray-200">
        <a href="<?= htmlspecialchars(URL . $baseProvas . '/resultado-aluno/' . (int)$prova['id'] . '/' . (int)$aluno['id'] . '/historico-respostas') ?>" target="_blank" rel="noopener" class="inline-flex items-center text-blue-600 hover:text-blue-800 font-medium">
...
<div class="bg-white rounded-xl shadow-lg p-6">
    ...
    <div class="space-y-6">
        <?php foreach ($questoes as $index => $questao): ?>
            ...
            $pontuacao = $resposta ? ($resposta['pontuacao'] ?? 0) : 0;
            ?>
