<?php
$base = $base_url_jornadas ?? (URL . '/admin/jornadas');
?>
<!-- Header -->
<div class="mb-8">
    <div class="flex justify-between items-center">
        ...
        <div>
            <a href="<?= $base ?>/<?= $jornada['id'] ?>/exercicios-alunos"
...
<?php if (empty($exercicios)): ?>
    <div class="bg-white rounded-xl shadow-lg p-12 border border-gray-200 text-center">
        ...
        <a href="<?= $base ?>/<?= $jornada['id'] ?>/modulos"
...
<div class="space-y-6">
    <?php foreach ($exercicios as $index => $exercicio): ?>
        <?php
        $respostaAluno = $exercicio['resposta_aluno'] ?? '';
        $pontuacao = $exercicio['pontuacao_aluno'] ?? 0;
        $correto = $pontuacao > 0;
        $respondido = !empty($respostaAluno);
        ...
        ?>
        <div class="bg-white rounded-xl shadow-lg p-6 border-2 <?= $correto ? 'border-green-200' : ($respondido ? 'border-red-200' : 'border-gray-200') ?>">
                ...
                <?php if ($respondido): ?>
                    <div class="text-right">
                        ...
                        <p class="text-2xl font-bold <?= $correto ? 'text-green-600' : 'text-red-600' ?>">
                            <?= number_format($pontuacao, 1) ?> / <?= number_format($exercicio['pontuacao'] ?? 1, 1) ?>
                        </p>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <!-- Resposta do Aluno (dissertativas) -->
            <?php if ($exercicio['tipo'] === 'dissertativa' && $respondido): ?>
                ...
                $notaAtual = $pontuacao !== null && $pontuacao !== '' ? (float) $pontuacao : 0;
                ?>
                ...
                <div class="bg-gray-50 rounded-lg p-4 mb-4 border border-gray-200" data-exercicio-dissertativa="<?= $exercicio['id'] ?>">
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <button type="button"
                                class="btn-primary-custom salvar-nota-dissertativa px-4 py-2 rounded-lg transition-colors text-sm font-medium hover:opacity-90"
                                data-exercicio-id="<?= $exercicio['id'] ?>"
...
(function() {
    ...
    var urlAtribuirNota = '<?= htmlspecialchars($url_atribuir_nota ?? (URL . '/professor/jornadas/exercicios/atribuir-nota-dissertativa'), ENT_QUOTES, 'UTF-8') ?>';
