    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600">
                Atualize os dados da matéria: <strong><?= htmlspecialchars($subject['nome']) ?></strong>
            </p>
        </div>
    ...
    <form id="subjectForm" class="p-6 space-y-6">
        ...
        <div>
            ...
            <input type="text" id="nome" name="nome" required
                   value="<?= htmlspecialchars($subject['nome']) ?>"
        ...
        <div class="bg-gray-50 p-4 rounded-lg">
            <h4 class="text-sm font-semibold text-gray-700 mb-2">Informações da Matéria</h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                <div>
                    <span class="font-medium">Data de Criação:</span> <?= date('d/m/Y H:i', strtotime($subject['created_at'])) ?>
                </div>
                <div>
                    <span class="font-medium">Última Atualização:</span> <?= date('d/m/Y H:i', strtotime($subject['updated_at'])) ?>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end space-x-4">
            ...
            <button type="submit" 
                ...
                <span id="submitText">Atualizar Matéria</span>
...
<script>
    document.getElementById('subjectForm').addEventListener('submit', async function(e) {
        ...
        try {
            // Adiciona método PUT ao FormData
            formData.append('_method', 'PUT');
            
            const response = await fetch('<?= URL ?>/admin/subjects/<?= $subject['id'] ?>', {
                method: 'POST',
                body: formData
            });
            ...
            if (response.ok) {
                ...
                errorDiv.textContent = result.error || 'Erro na atualização';
