    ...
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 mb-2">
                Editar Jornada: <?= htmlspecialchars($jornada['titulo']) ?>
            </h2>
            <p class="text-gray-600">
                Modifique os dados da jornada de aprendizado
            </p>
        </div>
...
<div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border border-blue-200">
    ...
    <form id="jornadaForm" class="p-6 space-y-6">
        ...
        <div>
            ...
            $turmasSelecionadas = $estrutura['turmas_selecionadas'] ?? [$jornada['turma_id']];
            ...
            <?php else: ?>
                ...
                <input type="hidden" id="turma_id" name="turma_id" value="<?= htmlspecialchars($jornada['turma_id']) ?>">
            <?php endif; ?>
        ...
        <div id="selecaoAlunos" class="<?= !empty($turmasSelecionadas) ? '' : 'hidden' ?>">
            <label class="block text-sm font-semibold text-gray-700 mb-2">
                Alunos
            </label>
            <div class="mb-3">
                <label class="flex items-center space-x-2 cursor-pointer">
                    <input type="radio" name="tipo_selecao_alunos" value="todos" 
                           <?= $tipoSelecaoAlunos === 'todos' ? 'checked' : '' ?>
                ...
                <label class="flex items-center space-x-2 cursor-pointer mt-2">
                    <input type="radio" name="tipo_selecao_alunos" value="especificos"
                           <?= $tipoSelecaoAlunos === 'especificos' ? 'checked' : '' ?>
                           class="text-blue-600 focus:ring-blue-500 tipo-selecao-alunos">
                    <span class="text-sm text-gray-700">Selecionar alunos específicos</span>
                </label>
            </div>
            <div id="containerAlunos" class="<?= $tipoSelecaoAlunos === 'especificos' ? '' : 'hidden' ?> border border-gray-300 rounded-lg p-4 max-h-64 overflow-y-auto bg-white">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
                ...
                $anoJornada = (int) ($jornada['ano_letivo'] ?? date('Y'));
                $bimJornada = (int) ($jornada['bimestre'] ?? 0);
                ...
                <select id="bimestre" name="bimestre" required
                        data-periodo-letivo-select data-periodo-vazio="1" data-periodo-selecionado="<?= $bimJornada ?>"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white text-gray-900 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                    <?= PeriodoLetivo::optionsHtml($anoJornada, $bimJornada, ['vazio' => true]) ?>
                </select>
            </div>
            <div>
                ...
                <select id="avaliativo" name="avaliativo"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white text-gray-900 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                    <option value="1" <?= (int)($jornada['avaliativo'] ?? 1) === 1 ? 'selected' : '' ?>>Sim</option>
                    <option value="0" <?= (int)($jornada['avaliativo'] ?? 1) === 0 ? 'selected' : '' ?>>Não</option>
                </select>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div>
                ...
                <input type="date" id="data_inicio" name="data_inicio" required
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white text-gray-900 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                       value="<?= htmlspecialchars($jornada['data_inicio']) ?>">
            </div>
            <div>
                ...
                <input type="time" id="hora_inicio" name="hora_inicio"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white text-gray-900 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                       value="<?= htmlspecialchars($jornada['hora_inicio'] ?? '') ?>">
            </div>
            <div>
                ...
                <input type="date" id="data_fim" name="data_fim" required
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white text-gray-900 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                       value="<?= htmlspecialchars($jornada['data_fim']) ?>">
            </div>
            <div>
                ...
                <input type="time" id="hora_fim" name="hora_fim"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white text-gray-900 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                       value="<?= htmlspecialchars($jornada['hora_fim'] ?? '') ?>">
            </div>
        </div>
        ...
        <div class="flex justify-end space-x-4">
            ...
            <button type="submit" 
                ...
                <span id="submitText">Atualizar Jornada</span>
                <span id="loadingText" class="hidden">Atualizando...</span>
            </button>
        </div>
    </form>
</div>
...
<script>
    // No formulário de edição, permitir datas anteriores (não definir data mínima)
    ...
    async function carregarAlunos(turmasIds) {
        ...
        try {
            ...
            if (data.success && data.alunos) {
                listaAlunos.innerHTML = '';
                const alunosSelecionados = <?= json_encode($alunosSelecionados) ?>;
                if (data.alunos.length === 0) {
                    ...
                    data.alunos.forEach(aluno => {
                        ...
                        const checked = alunosSelecionados.includes(parseInt(aluno.id)) ? 'checked' : '';
                        label.innerHTML = `
                            <input type="checkbox" name="alunos_id[]" value="${aluno.id}"
                                   class="rounded border-gray-300 text-blue-600 focus:ring-blue-500 aluno-checkbox" ${checked}>
    ...
    // Carregar alunos iniciais se houver turmas selecionadas
    const turmasIniciais = Array.from(document.querySelectorAll('.turma-checkbox:checked'))
        .map(cb => cb.value);
    if (turmasIniciais.length > 0) {
        carregarAlunos(turmasIniciais);
    }
    ...
    document.getElementById('jornadaForm').addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch('<?= URL ?>/professor/jornadas/<?= $jornada['id'] ?>/atualizar', {
                method: 'POST',
                body: formData
            });
