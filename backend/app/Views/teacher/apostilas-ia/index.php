<div class="mb-6">
    ...
    <p class="text-gray-600 mt-1">Apostilas e materiais enviados pela coordenação, com assistente de IA para tirar dúvidas, gerar provas e slides.</p>
</div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5" id="gradeMateriais">
    <?php if (empty($items)): ?>
        <div class="col-span-full bg-white rounded-xl shadow border border-gray-200 p-6 text-center text-gray-500">
            Nenhum material disponível para você ainda.
        </div>
    <?php else: ?>
        <?php foreach ($items as $item): ?>
            ...
            $itemId = (int)$item['id'];
            $capaUrl = URL . '/professor/apostilas-ia/' . $itemId . '/capa';
            $pdfUrl = URL . '/professor/apostilas-ia/' . $itemId . '/pdf';
            $assistenteUrl = URL . '/professor/apostilas-ia/' . $itemId;
            ...
            <div class="bg-white rounded-xl shadow border border-gray-200 overflow-hidden flex flex-col" data-materia="<?= htmlspecialchars($disciplinaNome) ?>">
                ...
                <div class="p-4 flex flex-col flex-1">
                    ...
                    <p class="text-xs text-gray-500 mt-1">
                        <?= !empty($item['turma_nome']) ? htmlspecialchars($item['turma_nome']) : '—' ?> ·
