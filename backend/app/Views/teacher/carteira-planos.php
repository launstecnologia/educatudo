...
<div class="max-w-4xl mx-auto">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-xl shadow border border-gray-200 p-6 flex flex-col">
            ...
            <p class="text-2xl font-bold text-primary-600 mt-2"><?= (int)($p['creditos_mensais'] ?? 0) ?> <span class="text-sm font-normal text-gray-600">créditos/mês</span></p>
            <p class="text-sm text-gray-500 mt-1">R$ <?= number_format((float)($p['valor_mensal'] ?? 0), 2, ',', '.') ?>/mês</p>
            <form method="post" action="<?= URL ?>/professor/carteira/planos/assinar" class="mt-4">
                <input type="hidden" name="plano_id" value="<?= (int)($p['id'] ?? 0) ?>">
                <input type="hidden" name="_token" value="<?= htmlspecialchars($csrf_token ?? '') ?>">
                <button type="submit" class="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 text-sm font-medium">Assinar</button>
            </form>
        </div>
        <?php endforeach; ?>
    </div>
