...
$situacoes = ['presente' => 'Presente', 'falta' => 'Falta', 'falta_justificada' => 'Justificada', 'atraso' => 'Atraso'];
?>
...
<div class="max-w-7xl mx-auto space-y-6">
    <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
        <div>
            <a href="<?= URL ?>/professor/diario?data=<?= urlencode($aula['data_aula']) ?>" class="text-purple-700 hover:underline text-sm">← Voltar ao diário</a>
    ...
    <form method="post" action="<?= URL ?>/professor/diario/salvar" id="formChamada" class="space-y-6">
        ...
        <section class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
            ...
            <?php if (!empty($aula['plano_aula_id'])): ?>
                <p class="font-semibold text-purple-800 mt-3"><?= htmlspecialchars($aula['plano_titulo'] ?? 'Plano vinculado') ?></p>
                <div class="text-gray-700 mt-2 whitespace-pre-line"><?= htmlspecialchars(trim(strip_tags((string) ($aula['plano_conteudo'] ?? '')))) ?: 'Plano sem conteúdo descritivo.' ?></div>
                <a href="<?= URL ?>/professor/planos-aula/visualizar/<?= (int) $aula['plano_aula_id'] ?>" class="inline-block text-sm text-purple-700 hover:underline mt-3" target="_blank">Ver plano completo ↗</a>
            <?php else: ?>
                <p class="text-amber-800 bg-amber-50 border border-amber-200 rounded-lg p-3 mt-3">Nenhum plano encontrado para esta data, turma e matéria. A chamada ainda pode ser realizada normalmente.</p>
            <?php endif; ?>
        </section>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs uppercase text-gray-500 w-28">Nota</th>
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    <input type="number" min="0" max="10" step="0.1"
                                        name="frequencias[<?= $aid ?>][nota]"
                                        value="<?= htmlspecialchars((string) ($freq['nota'] ?? '')) ?>"
