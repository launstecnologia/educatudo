        ...
        <ul class="space-y-2">
            <?php foreach ($items as $item): ?>
                <li class="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                    ...
                    <a href="<?= URL ?>/aluno/apostilas/abrir/<?= rawurlencode((string)$item['id']) ?>" class="shrink-0 ml-3 bg-indigo-600 text-white px-3 py-1.5 rounded-lg hover:bg-indigo-700 text-sm font-medium">
                        Abrir
                    </a>
                </li>
            <?php endforeach; ?>
