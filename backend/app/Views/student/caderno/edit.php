...
// Sempre usa editor HTML/JS (Fabric.js) — sem React/Excalidraw
...
<div class="container mx-auto px-4 py-6 max-w-7xl w-full">
    <div class="mb-6">
        <a href="<?= URL ?>/caderno/<?= (int)($caderno['id'] ?? 0) ?>" class="text-green-600 hover:text-green-700 font-medium flex items-center gap-1 mb-4">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
            Voltar à anotação
        </a>
        <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
            <h1 class="text-2xl font-bold text-gray-900">Editar anotação</h1>
            <a href="<?= URL ?>/caderno/<?= (int)($caderno['id'] ?? 0) ?>/excalidraw-editor" class="px-4 py-2 rounded-lg border border-violet-500 text-violet-700 hover:bg-violet-50 font-medium inline-flex items-center gap-2" target="_blank" rel="noopener">
    ...
    <div class="caderno-wrapper">
        <form method="POST" action="<?= URL ?>/caderno/<?= (int)$caderno['id'] ?>" enctype="multipart/form-data" class="space-y-5" id="form-caderno">
            <input type="hidden" name="_token" value="<?= htmlspecialchars($csrf_token) ?>">
            <textarea id="observacao" name="observacao" style="display: none;"><?= htmlspecialchars($observacaoRaw) ?></textarea>

            <div>
                <label for="titulo" class="block text-sm font-semibold text-gray-700 mb-2">Título *</label>
                <input type="text" id="titulo" name="titulo" required value="<?= htmlspecialchars($caderno['titulo'] ?? '') ?>"
                ...
                <div>
                    ...
                    <select id="pasta_id" name="pasta_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white/80">
                        ...
                        <?php foreach ($pastas as $p): ?>
                            <option value="<?= (int)$p['id'] ?>" <?= (int)($caderno['pasta_id'] ?? 0) === (int)$p['id'] ? 'selected' : '' ?>><?= htmlspecialchars($p['nome']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div>
                    ...
                    <select id="materia_id" name="materia_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white/80">
                        ...
                        <?php foreach ($materias as $m): ?>
                            <option value="<?= (int)$m['id'] ?>" <?= (int)($caderno['materia_id'] ?? 0) === (int)$m['id'] ? 'selected' : '' ?>><?= htmlspecialchars($m['nome']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            ...
            <?php if (!empty($anexos)): ?>
                <div>
                    <span class="block text-sm font-semibold text-gray-700 mb-2">Anexos atuais</span>
                    <ul class="space-y-2 text-sm text-gray-600">
                        <?php foreach ($anexos as $a): ?>
                            <li class="flex items-center justify-between bg-gray-50 rounded px-3 py-2">
                                <a href="<?= URL ?>/caderno/<?= (int)$caderno['id'] ?>/anexo/<?= (int)$a['id'] ?>" target="_blank" class="truncate flex-1"><?= htmlspecialchars($a['nome_original']) ?></a>
                                <button type="button" class="excluir-anexo ml-2 px-2 py-1 text-red-600 hover:bg-red-50 rounded" data-anexo-id="<?= (int)$a['id'] ?>">Excluir</button>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </div>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Adicionar mais anexos</label>
                <input type="file" name="anexos[]" multiple accept="image/*,.pdf,.doc,.docx,.txt"
                       class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:bg-green-50 file:text-green-700">
            </div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="<?= URL ?>/caderno/<?= (int)$caderno['id'] ?>" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-medium transition-colors">
                    Cancelar
                </a>
            </div>
        </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var observacaoVal = (document.getElementById('observacao') && document.getElementById('observacao').value) || '';
    var payload = null;
    try {
        if (observacaoVal.trim().charAt(0) === '{') payload = JSON.parse(observacaoVal);
    } catch (e) {}
    ...
    var alt = 500;
    if (payload && typeof payload.canvasHeight === 'number' && payload.canvasHeight >= 400 && payload.canvasHeight <= 1200) alt = payload.canvasHeight;
    var ALTURA = alt;

    var selAltura = document.getElementById('altura-quadro');
    if (selAltura) selAltura.value = String(ALTURA);
    ...
    if (payload && payload.canvas) {
        canvasU.loadFromJSON(payload.canvas, function() { canvasU.renderAll(); });
    }
    if (payload && typeof payload.texto === 'string' && payload.texto.trim() !== '') {
        var txtBlock = new fabric.IText(payload.texto.trim(), { left: 20, top: 20, fontFamily: 'Arial', fontSize: 18, fill: '#1f2937' });
        canvasU.add(txtBlock);
        canvasU.renderAll();
    }
    // Texto puro (sem JSON): coloca no canvas para o aluno continuar editando
    if (!payload && observacaoVal.trim() !== '') {
        var txtBlock = new fabric.IText(observacaoVal.trim(), { left: 20, top: 20, fontFamily: 'Arial', fontSize: 18, fill: '#1f2937' });
        canvasU.add(txtBlock);
        canvasU.renderAll();
    }

    selAltura && selAltura.addEventListener('change', function() {
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if (obj) { e.preventDefault(); canvasU.remove(obj); canvasU.renderAll(); }
    });

    var ferramentaU = 'caneta', corU = document.getElementById('cor-u').value, espessuraU = 4;
    ...
    function pararDesenhoU() { canvasU.isDrawingMode = false; }
    ...
    document.addEventListener('paste', function(e) {
        ...
        for (var i = 0; i < items.length; i++) {
            if (items[i].type.indexOf('image') !== -1) { e.preventDefault(); adicionarImagemAoCanvas(items[i].getAsFile()); return; }
        }
    });
                ...
                for (var i = 0; i < data.length; i++) {
                    for (var j = 0; j < data[i].types.length; j++) {
                        if (data[i].types[j].indexOf('image') !== -1) {
                            data[i].getType(data[i].types[j]).then(function(blob) { adicionarImagemAoCanvas(blob); });
                            return;
                        }
                    }
                }
                alert('Nenhuma imagem na área de transferência.');
            }).catch(function() { alert('Use Ctrl+V quando tiver uma imagem copiada.'); });
        } else { alert('Use Ctrl+V para colar uma imagem.'); }
    };
    document.getElementById('btn-adicionar-img').onclick = function() { document.getElementById('input-imagem').click(); };
    document.getElementById('input-imagem').onchange = function() {
        var f = this.files && this.files[0];
        if (f && f.type.indexOf('image') === 0) adicionarImagemAoCanvas(f);
        this.value = '';
    };
    ...
    document.querySelectorAll('.tool-btn-u[data-tool]').forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            if (ferramentaU === tool) { ferramentaU = 'selecionar'; pararDesenhoU(); }
            else {
                ferramentaU = tool;
                pararDesenhoU();
                if (tool === 'caneta' || tool === 'destaque') iniciarDesenhoU();
                if (tool === 'apagar') { var obj = canvasU.getActiveObject(); if (obj) { canvasU.remove(obj); canvasU.renderAll(); } }
            }
            atualizarBotoesU();
        });
    });
    ...
    document.getElementById('espessura-u').oninput = function() { espessuraU = parseInt(this.value, 10); if (canvasU.isDrawingMode && ferramentaU !== 'destaque') canvasU.freeDrawingBrush.width = espessuraU; };
    ...
    canvasU.on('mouse:up', function() {
        ...
        if (comprimento < 2) { canvasU.remove(linhaU); linhaU = null; }
        else {
            var ang = Math.atan2(dy, dx), angDeg = (ang * 180 / Math.PI) + 90;
            var tri = new fabric.Triangle({ width: 16, height: 16, fill: corU, left: fimX - Math.cos(ang) * 8, top: fimY - Math.sin(ang) * 8, originX: 'center', originY: 'center', angle: angDeg });
            canvasU.add(tri);
            linhaU = null;
        }
        canvasU.renderAll();
    ...
    canvasU.on('mouse:down', function(o) {
        ...
        circlePreviewU = new fabric.Circle({ left: p.x, top: p.y, radius: 0, stroke: corU, strokeWidth: Math.max(2, espessuraU), fill: 'transparent', selectable: false, evented: false });
    ...
    canvasU.on('mouse:up', function() {
        ...
        canvasU.remove(prev);
        circlePreviewU = null;
        if (r >= 3) canvasU.add(new fabric.Circle({ left: prev.left, top: prev.top, radius: r, stroke: corU, strokeWidth: Math.max(2, espessuraU), fill: 'transparent' }));
        canvasU.renderAll();
    });
    ...
    canvasU.on('mouse:down', function(o) {
        ...
        ovalPreviewU = new fabric.Ellipse({ left: p.x, top: p.y, rx: 0, ry: 0, stroke: corU, strokeWidth: Math.max(2, espessuraU), fill: 'transparent', selectable: false, evented: false });
    ...
    canvasU.on('mouse:up', function() {
        ...
        canvasU.remove(prev);
        ovalPreviewU = null;
        if (rx >= 3 || ry >= 3) canvasU.add(new fabric.Ellipse({ left: prev.left, top: prev.top, rx: rx, ry: ry, stroke: corU, strokeWidth: Math.max(2, espessuraU), fill: 'transparent' }));
        canvasU.renderAll();
    });
    ...
    canvasU.on('mouse:down', function(o) {
        ...
        var p = canvasU.getPointer(o.e);
        var txt = new fabric.IText('Digite seu texto aqui no quadro', { left: p.x, top: p.y, fontFamily: 'Arial', fontSize: 18, fill: corU });
        canvasU.add(txt);
    ...
    document.getElementById('form-caderno').addEventListener('submit', function() {
        var payload = { canvas: canvasU.toObject(['data']), texto: '', canvasHeight: ALTURA };
...
<script>
document.querySelectorAll('.excluir-anexo').forEach(function(btn) {
    btn.addEventListener('click', function() {
        if (!confirm('Excluir este anexo?')) return;
        var anexoId = this.dataset.anexoId;
        var form = new FormData();
        form.append('_token', '<?= htmlspecialchars($csrf_token) ?>');
        form.append('anexo_id', anexoId);
        fetch('<?= URL ?>/caderno/anexo/excluir', { method: 'POST', body: form })
