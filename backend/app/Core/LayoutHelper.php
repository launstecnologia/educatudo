...
class LayoutHelper
{
    ...
    public static function renderComentarioGabarito($texto): string
    {
        ...
        if ($decoded !== $texto) {
            $texto = html_entity_decode($decoded, ENT_QUOTES | ENT_HTML5, 'UTF-8');
        }

        $texto = strip_tags($texto);
        $texto = preg_replace('/[ \t]+/u', ' ', $texto) ?? $texto;
        $texto = preg_replace('/\s*---\s*/u', "\n\n", $texto) ?? $texto;
        $texto = preg_replace('/^Resolu[cç][aã]o\s+Passo\s+a\s+Passo\s*/iu', "Resolução passo a passo\n\n", $texto) ?? $texto;
        $texto = preg_replace('/\s+(Passo\s+\d+\s*:)/iu', "\n\n$1", $texto) ?? $texto;
        $texto = preg_replace('/\s+(An[áa]lise\s+das\s+Alternativas[^:]*:)/iu', "\n\n$1", $texto) ?? $texto;
        $texto = preg_replace('/\s*-\s*(Alternativa\s+[A-E]\s*(?:\([^)]+\))?\s*:)/iu', "\n$1", $texto) ?? $texto;
        ...
        foreach ($blocos as $bloco) {
            ...
            if (preg_match('/^Resolu[cç][aã]o\s+passo\s+a\s+passo$/iu', $bloco)) {
                $html .= '<h4 class="text-sm font-semibold text-slate-900">Resolução passo a passo</h4>';
                continue;
            }

            if (preg_match('/^(Passo\s+\d+)\s*:\s*(.*)$/isu', $bloco, $m)) {
                $titulo = htmlspecialchars($m[1], ENT_QUOTES, 'UTF-8');
                $corpo = nl2br(htmlspecialchars(trim($m[2]), ENT_QUOTES, 'UTF-8'));
                $html .= '<div class="rounded-lg border border-slate-200 bg-white p-3">';
                $html .= '<p class="text-xs font-semibold uppercase text-blue-700 mb-1">' . $titulo . '</p>';
                $html .= '<p class="text-sm leading-relaxed text-slate-700">' . $corpo . '</p>';
                $html .= '</div>';
                continue;
            }

            if (preg_match('/^(An[áa]lise\s+das\s+Alternativas[^:]*:)\s*(.*)$/isu', $bloco, $m)) {
                $titulo = htmlspecialchars(rtrim($m[1], ':'), ENT_QUOTES, 'UTF-8');
                $corpo = nl2br(htmlspecialchars(trim($m[2]), ENT_QUOTES, 'UTF-8'));
                $html .= '<div class="pt-2 border-t border-slate-200">';
                $html .= '<p class="text-sm font-semibold text-slate-900 mb-2">' . $titulo . '</p>';
                if ($corpo !== '') {
                    $html .= '<p class="text-sm leading-relaxed text-slate-700">' . $corpo . '</p>';
                }
                $html .= '</div>';
            ...
            if (count($linhas) > 1) {
                ...
                foreach ($linhas as $linha) {
                    ...
                    if (preg_match('/^(Alternativa\s+[A-E]\s*(?:\([^)]+\))?\s*:)\s*(.*)$/isu', $linha, $m)) {
                        $html .= '<p class="text-sm leading-relaxed text-slate-700"><span class="font-semibold text-slate-900">' . htmlspecialchars($m[1], ENT_QUOTES, 'UTF-8') . '</span> ' . htmlspecialchars(trim($m[2]), ENT_QUOTES, 'UTF-8') . '</p>';
                    } else {
                        $html .= '<p class="text-sm leading-relaxed text-slate-700">' . htmlspecialchars($linha, ENT_QUOTES, 'UTF-8') . '</p>';
                    }
                }
                $html .= '</div>';
                continue;
            ...
            $html .= '<p class="text-sm leading-relaxed text-slate-700">' . nl2br(htmlspecialchars($bloco, ENT_QUOTES, 'UTF-8')) . '</p>';
        }
